export function bookingSummaryKey(summary) {
  if (!summary) {
    return null;
  }
...
export function createBookingSummaryElement({ template, summary, labelForField }) {
...
export function createBookingSummaryHistory({ template, messageContainer, labelForField, appendSummary }) {
  ...
  function append(summary) {
    ...
    const entry = typeof appendSummary === "function"
      ? appendSummary(summary)
      : appendSummaryToContainer(summary);
  ...
  function appendSummaryToContainer(summary) {
    const entry = createBookingSummaryElement({
      template,
      summary,
      labelForField,
    });
