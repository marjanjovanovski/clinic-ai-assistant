...
| 9. Should unavailable calendar days be disabled? | Yes, but only when backend confidence is reliable; otherwise leave them clickable and return `No available slots` with alternatives. | False-disabled dates are worse than graceful no-availability responses. | `Agreed` |
| 10. How should slot conflict recovery behave? | Immediately explain the conflict and offer replacement slots or refreshed availability in the same response. | Recovery should feel fast and continuous, not like a restart. | `Agreed, but we have to be carefull, i already think there are features in the code that support this, have to be checked, do not create duplicate code/functions` |
| 11. What is the long-term role of the 3-day widget? | Keep it as the default quick-view widget, but not as the only scheduling surface once the date-picker exists. | It is useful for speed, but too narrow as a complete scheduling interface. | `Agreed` |
| 12. What should the LLM own versus backend logic? | LLM owns intent recognition and reply framing; backend owns slot truth, routing, handoff, and conflict recovery. | This avoids hallucinated availability and keeps behavior deterministic. | `Agreed` |
| 13. What should move out of `ai_agent.py` first? | Extract scheduling entry logic, availability response shaping, handoff preparation, widget payload assembly, and conflict recovery into a scheduling service layer. | These are the clearest scheduling-owned behaviors and reduce orchestration sprawl. | `Agreed, btw do you think that what will come out of this brainstorm session will have to produce prompts that will be better to be split by tasks as i see quite a lot of features we have to do. Dynamic Flow configuration brainstorm.md is a result of this point for the time being` |
| 14. Where should copy, labels, and behavior knobs live? | In JSON config from the start, not in Python logic. | This keeps the architecture aligned with your config-first rule. | `Agreed` |
| 15. What should be explicitly avoided? | Avoid hard-coded phrase dictionaries, premature booking prompts, widget-only scheduling, and duplicated scheduling logic across files. | These are the main paths that would weaken both architecture and UX. | `Agreed` |
| 16. What should be the first implementation priority? | Lock intent routing, direct typed-date answers, scheduling-to-booking handoff, and slot-conflict recovery before adding wider UX polish. | These produce the biggest functional and product improvement first. | `Agreed` |

## Continued Brainstorming Decision Table

| Question | Recommended answer | Why | Your final decision |
|---|---|---|---|
| 17. When should service or category become mandatory before scheduling can continue? | Only when the backend cannot return valid availability without knowing the requested service or category. | This preserves independence between flows while still protecting correctness when slot rules differ by service. | `Agreed` |
| 18. For broad-range requests like `next week`, what should the system do first? | Confirm that availability exists in that range and ask the user to narrow by date, instead of dumping many slots immediately. | This keeps the reply useful without overwhelming the user and fits well with a later calendar widget path. | `Agreed` |
| 19. If broad-range availability does not exist, what should the recovery look like? | Return a truthful no-availability assessment for the range and offer the nearest alternative range or earliest available date. | This keeps the system honest while still moving the conversation forward. | `Agreed` |
| 20. Should broad-range requests ever return sample slots immediately? | Yes, but only when the result set is naturally small and readable; otherwise ask the user to narrow first. | This avoids clutter while still allowing fast-path usefulness in simple cases. | `Agreed` |
| 21. After answering a typed specific date, should we always attach a widget too? | No. Attach a widget only when it materially helps the next step, such as no availability, multiple alternatives, or further exploration. | This keeps the response focused and avoids overloading the chat with UI every time. | `Agreed` |
| 22. How should ambiguous relative dates like `next Tuesday` be resolved? | Resolve them automatically when the backend can do so safely under one rule; otherwise ask one short clarification. | This keeps behavior natural while avoiding hidden date assumptions. | `Agreed` |
| 23. How should ambiguous time-window phrases like `afternoon` be resolved? | Map them to backend-defined default filters, for example a clinic-defined afternoon range in config. | This creates consistency and avoids repeated custom logic in Python. | `Agreed` |
| 24. What should the temporary-hold UX include after slot selection? | Show the selected slot clearly, enter booking immediately, and expose a low-friction path to change the slot while the hold is still active. | This keeps booking momentum without trapping the user. | `Agreed` |
| 25. If the user backs out after selecting a slot, what should happen to the temporary hold? | Keep the hold active for its normal lifetime unless the user explicitly cancels or the backend releases it. | This gives the user room to continue naturally without losing the selected slot too fast. | `I think there is already logic for this in the code. Be mindfull that no duplicates are created from code perspective, analyse before prompting and development is the flag here` |
| 26. What is the safest first extraction boundary from `ai_agent.py`? | Extract scheduling entry assessment, availability response shaping, booking handoff payload preparation, widget payload assembly, and slot-conflict recovery, while keeping top-level orchestration in `ai_agent.py` initially. | This reduces risk by moving clearly scheduling-owned behavior first without over-refactoring the whole agent lifecycle. | `Agreed, i have a note here, this might be considered as the first things that should happen as it might produce dependencies with the continued development work to achieve this task.` |
| 27. Should the first extraction task also redesign orchestration, or mainly relocate logic? | Mainly relocate and clarify logic first; broader orchestration redesign should come later if still needed. | This limits regression risk and keeps the first extraction task verifiable. | `Agreed` |
| 28. How should we handle existing slot-conflict logic during implementation? | Audit and reuse existing support wherever possible, and only add code where the current behavior is incomplete. | This avoids duplication and respects the partial recovery behavior already in the repo. | `I think there is logic already for this, analyse before prompting and development. If exists it needs only to be restructured in the new approach of segmenting the flows in separate serivecs separate py files` |
| 29. Should scheduling-specific config be introduced in the first implementation? | Yes, but narrowly: only the labels, messages, and simple behavior knobs needed by the new scheduling contract. | This respects the config-first rule without expanding into postponed dynamic-flow design. | `Agreed` |
| 30. What should be the execution order across implementation tasks? | First lock the scheduling contract, then extract scheduling logic from `ai_agent.py`, then stabilize the slot widget, and only after that handle later follow-ups like month calendar. | Later work depends on the behavior contract and extraction boundary being stable first. | `Agreed` |
