...
|---|---|---|
| 1 | Start a booking flow that asks for contact details | The backend returns a field prompt from the system message catalog rather than a hard-coded literal path |
| 2 | Enter an invalid phone or email value | The retry message appears correctly and matches the configured locale |
| 3 | Trigger a scheduling conflict path such as a lost slot | The conflict message appears correctly and matches the configured locale |
| 4 | Switch to another configured locale if supported in scope | The same backend state returns the translated system message without code changes |
| 5 | Review tenant profile-driven replies such as service descriptions or greetings | Those replies remain unchanged and are still sourced from tenant profile behavior, not the new system catalog |
